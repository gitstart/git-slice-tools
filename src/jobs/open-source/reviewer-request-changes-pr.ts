...
export const reviewerRequestChangesPR = async (
    actionInputs: ActionInputs,
    reviewer: string,
    repoName: string,
    prNumber: number
): Promise<void> => {
    logger.logInputs('open-source reviewer-request-changes-pr', { reviewer, repoName, prNumber })
    ...
    const statusChangesRequestedOption = statusField.options?.find(
        x => x.name === OPEN_SOURCE_STATUS_OPTIONS.ChangesRequested
    )
    ...
    logger.logWriteLine('OpenSource', `Updating 'Status=<Changes requested>'...`)
    await updateIssueFieldValue(
        ...
        statusField.id,
        'SingleSelect',
        statusChangesRequestedOption?.id
    )
