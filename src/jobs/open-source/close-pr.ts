...
export const closePR = async (
    ...
    logger.logInputs('open-source close-pr', { repoName, prNumber, maintainer })
    ...
    const statusPRClosedOption = statusField.options?.find(x => x.name === OPEN_SOURCE_STATUS_OPTIONS.PRClosed)

    const requiredStatus: string[] = [
        OPEN_SOURCE_STATUS_OPTIONS.InProgress,
        OPEN_SOURCE_STATUS_OPTIONS.ChangesRequested,
        OPEN_SOURCE_STATUS_OPTIONS.ClientReview,
        OPEN_SOURCE_STATUS_OPTIONS.ReadyForLastReview,
        OPEN_SOURCE_STATUS_OPTIONS.ReadyToPushUpstream,
    ]
    ...
    logger.logWriteLine('OpenSource', `Updating 'Status=PR closed'...`)
