...
const loadActionInputsAndInit = async (
    ...
    logger.logWriteLine('Common', 'Initialized git instances')
...
export default (args: string[] = null) =>
    yargs(args ?? process.argv.slice(2))
        ...
        .command('pull', 'Pull last changes from upstream repo into slice repo', GLOBAL_OPTIONS_CONFIG, ({ env }) => {
            return loadActionInputsAndInit(env, ({ sliceGit, upstreamGit, actionInputs }) =>
        ...
        .command(
            ...
            ({ env, prNumber, prReivewLink }) => {
                ...
                return loadActionInputsAndInit(env, ({ actionInputs }) =>
                    pullReview(actionInputs, prNumber, prReivewLink)
                )
            }
        )
        .command(
        ...
        .command('open-source', 'Open source tools', openSourceArgv => {
            return openSourceArgv
                ...
                .command(
                    ...
                    argv => {
                        return openSource.setupWorkflow(argv['dir'])
                    }
                )
        })
